...
var _roomJson = __webpack_require__(5);
...
var RoomServer = function () {
     ...
     */
    function RoomServer(hypertyURL, bus, configuration) {
        _classCallCheck(this, RoomServer);
        ...
        if (configuration["lwm2mUrl"]) url = configuration["lwm2mUrl"];
        ...
        this.contextSchemaURL = 'hyperty-catalogue://catalogue.' + (0, _utils.divideURL)(hypertyURL).domain + '/.well-known/dataschema/Context';

        // make hyperty discoverable
        this.discovery = new _Discovery2.default(hypertyURL, configuration.runtimeURL, bus);

        // syncher
        this.syncher = new _Syncher.Syncher(hypertyURL, bus, configuration);
    ...
    _createClass(RoomServer, [{
        key: "setupRemoteFunctionCallAbility",
        value: function setupRemoteFunctionCallAbility() {
            var _this2 = this;
            ...
            //add listener
            this.bus.addListener(this.hypertyURL, function (msg) {
                l.d("got a message!:", msg);
                // we are only interested in execute messages
                if (msg.type === "execute") {
                    l.d("got execute message:", msg);
                    var func = _this2.remoteFunctions[msg.body.method];
                    ...
                    if (func) {
                        ...
                        try {
                            ...
                            var result = func.apply(_this2, msg.body.params);
        ...
        value: function polling() {
            ...
            this.getRooms().then(function (roomsArray) {
                ...
                roomsArray.forEach(function (room) {
                    room.devices.forEach(function (device) {
                        for (var key in device.lastValues) {
                            device.lastValues[key].forEach(function (devObj) {
                    ...
                    var oldRoomDate = _this3.lastDateMap[room.name];

                    if (!oldRoomDate && !(room.name in _this3.roomMap)) {
                        // room doesn't exist yet
                        l.d("CREATING ROOM ", room.name);
                        _this3.setUpRoomSyncherObject(room);
                    } else if (oldRoomDate < dateMap[room.name]) {
                        // room must be updated
                        l.d("UPDATING ROOM ", room.name);
                        console.log(_this3.roomMap[room.name]);
                        try {
                            // this.roomMap[room.name].data = this.createRoomContext(room);
                            _this3.roomMap[room.name].data.values = _this3.createRoomContext(room).values;
                        } catch (e) {
        ...
        key: "addFunctionForRemote",
        value: function addFunctionForRemote(func) {
            var name = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : func.name;
        ...
        key: "sendResponse",
        value: function sendResponse(requestMsg, response) {
        ...
        key: "modifyRoom",
        value: function modifyRoom(roomNr) {
            var values = this.roomMap[roomNr].data.values;
            var val = values[0];
        ...
        key: "getRoomsForRemote",
        value: function getRoomsForRemote(token) {
        ...
        value: function getRooms() {
            if (useExampleRoomJson) {
                l.d("setUpRooms uses roomJson:", _roomJson2.default);
                return new Promise(function (resolve) {
                    resolve(_roomJson2.default.data);
                });
            } else {
                var json = { "mode": "read", "room": null };
                return this.makeRequest(json).then(function (respJson) {
        ...
        key: "setUpRoomSyncherObject",
        value: function setUpRoomSyncherObject(roomJson) {
            var _this4 = this;

            l.d("setUpRoomSyncherObject:", arguments);
            var context = this.createRoomContext(roomJson);
            l.d("context JSON created:", context);
            return this.syncher.create(this.contextSchemaURL, [this.hypertyURL], context).then(function (synchRoom) {
                l.d('SyncObject for room ' + context.name + ' created:', synchRoom);

                // react to added children
                synchRoom.onAddChild(function (child) {
                ...
                synchRoom.onSubscription(function (event) {
                ...
                _this4.roomMap[context.name] = synchRoom;
        ...
        key: "createRoomContext",
        value: function createRoomContext(roomJson) {
            ...
            roomJson.devices.forEach(function (device) {
                context.values.push({
