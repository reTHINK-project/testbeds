...
var CodeGeneratorObserver = function (_EventEmitter) {
  _inherits(CodeGeneratorObserver, _EventEmitter);

  function CodeGeneratorObserver(hypertyURL, bus, configuration) {
    _classCallCheck(this, CodeGeneratorObserver);

    if (!hypertyURL) throw new Error('The hypertyURL is a needed parameter');
    if (!bus) throw new Error('The MiniBus is a needed parameter');
    if (!configuration) throw new Error('The configuration is a needed parameter');

    var _this2 = _possibleConstructorReturn(this, (CodeGeneratorObserver.__proto__ || Object.getPrototypeOf(CodeGeneratorObserver)).call(this));
  ...
  _createClass(CodeGeneratorObserver, [{
    key: '_onNotification',
    value: function _onNotification(event) {
      ...
      _this._syncher.subscribe(_this._objectDescURL, event.url, true, false).then(function (codeGeneratorObserver) {
    ...
    key: '_changes',
    value: function _changes(codeGeneratorObserver) {
      var _this3 = this;

      codeGeneratorObserver.onChange('*', function (event) {

        // CodeGenerator Object was changed
        console.info('[codeGeneratorObserver.onChange]', event);

        // lets notify the App about the change
        _this3.trigger('code', codeGeneratorObserver.data);
      });
    }
  }]);

  return CodeGeneratorObserver;
