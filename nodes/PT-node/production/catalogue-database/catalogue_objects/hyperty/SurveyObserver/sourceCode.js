...
var _Syncher = __webpack_require__(7);
...
var SurveyObserver = {
    onRequest: function onRequest(callback) {
        ...
        this.syncher.onNotification(function (event) {
            if (event.schema === _this.objectDescURL && event.value.resources[0] === 'survey') {
                _this.syncher.subscribe(_this.objectDescURL, event.url).then(function (dataObject) {
                    dataObject.onAddChild(function (msg) {
                        callback({
                            answer: function answer(_answer) {
                                dataObject.addChild('resources', { response: _answer });
                            },
