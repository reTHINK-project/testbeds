...
var _Discovery = __webpack_require__(5);
...
var LocationObserverHyperty = function LocationObserverHyperty(descURL, syncher, discovery) {
    var users_position = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : [];
    var callback = arguments[4];

    var getUsersPosition = function getUsersPosition() {
        return users_position;
    };
    var watchUsersPosition = function watchUsersPosition(c) {
        callback = c;
        callback(users_position);
    };

    discovery.discoverDataObjectsPerName('location').then(function (dataobjects) {
        var liveDOs = dataobjects.filter(function (d) {
            return d.status === 'live';
        });
        console.log('[DiscoveryHyperty]', liveDOs);
        liveDOs.forEach(function (dataobject) {
            syncher.subscribe(descURL, dataobject.url).then(function (observer) {
                ...
                var position = {
                    ...
                    coords: {
                        latitude: observer.data.values.find(function (v) {
                            return v.name === 'latitude';
                        }).value,
                        longitude: observer.data.values.find(function (v) {
                            return v.name === 'longitude';
                        }).value
                    }
                };
                ...
                observer.onChange('*', function (event) {
                    if (event.field === 'values') {
                        position.coords.latitude = event.data.find(function (v) {
                            return v.name === 'latitude';
                        }).value;
                        position.coords.longitude = event.data.find(function (v) {
                            return v.name === 'longitude';
                        }).value;
                    }
                    if (callback) callback(users_position);
                });
                if (callback) callback(users_position);
            });
        });
    ...
    return { getUsersPosition: getUsersPosition, watchUsersPosition: watchUsersPosition };
};

function activate(hypertyURL, bus, config) {
    var uri = new _urijs2.default(hypertyURL);
    var _objectDescURL = 'hyperty-catalogue://catalogue.' + uri.hostname() + '/.well-known/dataschema/Context';
    var _syncher = new _Syncher.Syncher(hypertyURL, bus, config);
    var _discovery = new _Discovery2.default(hypertyURL, config.runtimeURL, bus); //TODO
