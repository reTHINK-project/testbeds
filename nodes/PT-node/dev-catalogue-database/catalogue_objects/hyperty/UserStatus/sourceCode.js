...
* @version 0.1.0
*/var UserStatus=function(_EventEmitter){_inherits(UserStatus,_EventEmitter);function UserStatus(hypertyURL,bus,configuration){_classCallCheck(this,UserStatus);if(!hypertyURL)throw new Error('The hypertyURL is a needed parameter');if(!bus)throw new Error('The MiniBus is a needed parameter');if(!configuration)throw new Error('The configuration is a needed parameter');var _this=_possibleConstructorReturn(this,Object.getPrototypeOf(UserStatus).call(this,hypertyURL,bus,configuration));_this._syncher=new _Syncher.Syncher(hypertyURL,bus,configuration);_this._hypertyDiscovery=new _HypertyDiscovery2.default(hypertyURL,bus);_this._userStatusDescURL='hyperty-catalogue://'+new _urijs2.default(hypertyURL).hostname()+'/.well-known/dataschemas/Context';_this._heartbeat=[];_this._syncher.onNotification(function(event){_this._onNotification(event);});return _this;}_createClass(UserStatus,[{key:'_onNotification',value:function _onNotification(event){var _this2=this;console.info('UserStatus Event Received: ',event);console.log('from hyperty',event.from);event.ack();if(event.schema===this._userStatusDescURL){ // Subscribe to user status presence
this._syncher.subscribe(this._userStatusDescURL,event.url).then(function(statusObjectObserver){console.info('-------- Status Observer received ---------',statusObjectObserver);console.log('trigger statusChange for',event.identity);_this2.trigger('statusChange',{identity:event.identity,status:_this2.getStatus(statusObjectObserver)});_this2._managePresenceHeartbeat(event.identity);statusObjectObserver.onChange('*',function(){console.info('status event received:',event);_this2.trigger('statusChange',{identity:event.identity,status:_this2.getStatus(statusObjectObserver)});_this2._managePresenceHeartbeat(event.identity);});console.log(event.identity.email,'has subscribe to my status data object, send invite to listen mine');_this2._statusObjectReporter.inviteObservers([event.from]);}).catch(function(reason){console.error(reason);});}} /**
   * This function is used to create a new status object syncher
   * @param  {URL.UserURL} participants List of User allowed
   * @return {Promise}
   */},{key:'create',value:function create(participants){var _this3=this;return new Promise(function(resolve,reject){console.info('----------------------- Mapping Particpants --------------------');_this3._mappingUser(participants).then(function(hyperties){return _this3.createSyncher(hyperties,(0,_availability2.default)());}).then(function(statusObjectReporter){_this3._statusObjectReporter=statusObjectReporter;_this3._statusObjectReporter.onSubscription(function(event){console.info('-------- Status Reporter received subscription request ---------',event);event.accept();if(event.url===_this3._statusObjectReporter._owner){console.log('define my own presence state to available');_this3.setStatus('available');}}); // set interval for heartbeat
setInterval(function(){_this3._sendHeartbeat();},50000);}).catch(function(reason){reject(reason);});});}},{key:'createSyncher',value:function createSyncher(hyperties,status){console.info('------------------------ Syncher Create ----------------------',hyperties,status);return this._syncher.create(this._userStatusDescURL,hyperties,status);}},{key:'join',value:function join(resource){var _this4=this;return new Promise(function(resolve,reject){console.info('------------------------ Syncher subscribe ----------------------');_this4._syncher.subscribe(_this4._userStatusDescURL,resource).then(function(dataObjectObserver){console.info('Data Object Observer: ',dataObjectObserver);}).catch(function(reason){reject(reason);});});}},{key:'getStatus',value:function getStatus(statusObserver){var state=false;if(typeof statusObserver!=='undefined'){console.log('getStatus from observer:',statusObserver.data.values[0].value);return statusObserver.data.values[0].value;}else {console.log('getStatus from reporter:',this._statusObjectReporter.data.values[0].value);return this._statusObjectReporter.data.values[0].value;}return state;}},{key:'setStatus',value:function setStatus(newStatus){console.log('setStatus',newStatus,'in reporter',this._statusObjectReporter.data);this._statusObjectReporter.data.values[0].value=newStatus;this._statusObjectReporter.data.time=new Date().getTime();} /**
   * Update status object date
   */},{key:'_sendHeartbeat',value:function _sendHeartbeat(){console.log('send heartbeat');this._statusObjectReporter.data.time=new Date().getTime();} /**
   * Monitor user activity within heartbeat timeout period
   */},{key:'_managePresenceHeartbeat',value:function _managePresenceHeartbeat(identity){var _this5=this;console.log('renew heartbeat period for',identity);var email=identity.email;if(email in this._heartbeat){clearTimeout(this._heartbeat[email]);}this._heartbeat[email]=setTimeout(function(){console.log(email,'has disconnect');clearTimeout(_this5._heartbeat[email]);_this5.trigger('statusChange',{identity:identity,status:'unavailable'});},60000);}},{key:'_mappingUser',value:function _mappingUser(userList){var _this6=this;console.info('------------------------ _mappingUser ----------------------',userList);return new Promise(function(resolve,reject){var hyperties=[];var count=0;if(userList.length===0)reject(hyperties);var resultUsers=function resultUsers(){if(count===userList.length){console.info('Have '+hyperties.length+'users found;');resolve(hyperties);}};var activeUsers=function activeUsers(user){count++;hyperties.push(user.hypertyURL);resultUsers();};var inactiveUsers=function inactiveUsers(){count++;resultUsers();};userList.forEach(function(user){console.log(user);if(user.email.length){console.info('------------------------ _mappingUser ----------------------',userList);return _this6._hypertyDiscovery.discoverHypertyPerUser(user.email,user.domain).then(activeUsers).catch(inactiveUsers);}});});}}]);return UserStatus;}(_EventEmitter3.default);function activate(hypertyURL,bus,configuration){return {name:'UserStatus',instance:new UserStatus(hypertyURL,bus,configuration)};}module.exports=exports['default'];
...
"use strict";Object.defineProperty(exports,"__esModule",{value:true});exports.default=function(){return Object.assign({},{schema:"context",id:'_'+Math.random().toString(36).substr(2,9),type:"presence",time:"0",values:[{name:"availability",unit:"availability",value:"unavailable"}]});};;module.exports=exports['default'];
