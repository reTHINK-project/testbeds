...
// version: 0.4.0
// Last build: Fri Oct 28 2016 12:27:19 GMT+0100 (WEST)
...
// version: 0.4.0
// Last build: Fri Oct 28 2016 12:27:20 GMT+0100 (WEST)
...
var GroupChatHyperty = {
    _getHyFor: function _getHyFor(participants) {
        ...
        return Promise.all(participants.map(function (p) {
            return _this.hypertyDiscoveryService.discoverHyperty(p.email, _this.scheme, _this.resources, p.domain).then(function (hyperties) {
                var target = Object.keys(hyperties).sort(function (a, b) {
                    return new Date(hyperties[a].lastModified) < new Date(hyperties[b].lastModified) ? 1 : -1;
                }).shift();
    ...
    _addChild: function _addChild(dataObject) {
        return function (message) {
            return dataObject.addChild('chatmessages', message);
        };
    },
    _onAddChild: function _onAddChild(dataObject) {
        return function (callback) {
            dataObject.onAddChild(function (child) {
                return callback(child.data || child.value, child.identity);
            });
        };
    },
    create: function create(name, participants) {
        ...
        return this._getHyFor(participants).then(function (hyperties) {
            return _this2.syncher.create(_this2.objectDescURL, hyperties, (0, _communication2.default)(name, _this2.hypertyURL, hyperties.concat([_this2.hypertyURL])));
        }).then(function (dataObjectReporter) {
            dataObjectReporter.onSubscription(function (event) {
                return event.accept();
            });
            return _this2.identityManagerService.discoverUserRegistered().then(function (identity) {
                return (0, _GroupChat2.default)(dataObjectReporter.url, _this2._addChild(dataObjectReporter), _this2._onAddChild(dataObjectReporter), dataObjectReporter.data, _this2._position.data, identity);
            });
        });
    },
    onInvite: function onInvite(callback) {
        ...
        this.syncher.onNotification(function (event) {
            if (event.schema.endsWith(_this3.locationDS)) {
                _this3.syncher.subscribe(_this3.locationDescURL, event.url).then(function (dataObject) {
                    _this3._position = dataObject;
                });
            } else if (event.schema.endsWith(_this3.groupChatDS)) {
                _this3.syncher.subscribe(_this3.objectDescURL, event.url).then(function (dataObject) {
                    return _this3.identityManagerService.discoverUserRegistered().then(function (identity) {
                        _this3.notifications.trigger([{ email: identity.username, domain: _this3.domain }], { type: 'NEW_CHAT', payload: { id: dataObject.url, name: dataObject.data.name } });
                        callback((0, _GroupChat2.default)(dataObject.url, _this3._addChild(dataObject), _this3._onAddChild(dataObject), dataObject.data, _this3._position.data, identity));
                    });
                });
            }
        });
...
var groupChatFactory = function groupChatFactory(hypertyURL, bus, config) {
    ...
    var hypertyDiscovery = new _Discovery2.default(hypertyURL, bus);
...
exports.default = function (id, addChild, onAddChild, _ref, position, identity) {
    var name = _ref.name,
...
var GroupChat = {
    sendMessage: function sendMessage(message, distance) {
        ...
        return this.addChild({ message: message, distance: distance, position: this.position, startingTime: Date.now() }).then(function (child) {
            _this.messages.push((0, _GroupChatMessage2.default)(child.data, true, _this.identity));
    ...
    onMessage: function onMessage(callback) {
        ...
        this.onAddChild(function (message, identity) {
            return _this2._processTextMessages(message, identity, callback);
        });
    },
    _processTextMessages: function _processTextMessages(message, identity, callback) {
        var myPosition = (0, _gps.Position)(this.position);
        var area = (0, _gps.Area)(message.position, message.distance);

        if (!myPosition.isIn(area)) return;

        this.messages.push((0, _GroupChatMessage2.default)(message, false, identity.userProfile));
...
exports.default = function (_ref, isMe, identity) {
...
function communication(name, url, participants) {
...
var Position = {
    ...
    isIn: function isIn(area) {
        if (!area.radius) return true;
...
function PositionFactory(positionArray) {
    ...
    if (positionArray.values && positionArray.values.forEach) positionArray.values.forEach(function (value) {
...
exports.Position = PositionFactory;
exports.Area = AreaFactory;
